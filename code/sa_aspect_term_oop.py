...
class Model:
    def __init__(self, batch_size, seq_max_len, nb_sentiment_label, embedding_size, nb_linear_inside,
        nb_lstm_inside, layers, TRAINING_ITERATIONS, LEARNING_RATE, WEIGHT_DECAY, flag_train):
    ...
    def modeling(self):
        self.tf_X_train = tf.placeholder(tf.float32, shape=[None, self.seq_max_len, self.embedding_size])
        ...
        self.ln_w = tf.Variable(
            tf.truncated_normal(
                [self.embedding_size, self.nb_linear_inside],
                stddev = 1.0 / math.sqrt(self.embedding_size)))
        self.ln_b = tf.Variable(tf.zeros([self.nb_linear_inside]))
        
        
        self.sent_w = tf.Variable(
            tf.truncated_normal(
                [self.nb_lstm_inside, self.nb_sentiment_label],
    ...
    def predict(self, sess, data, flag_write_to_file, flag_train):
        ...
        if (flag_write_to_file):
            f_predict = codecs.open('../result/predict.txt', 'w', 'utf-8')
            f_predict.write('#-------------------------------------------------------------------------------------#\n')
            f_predict.write('#\t author: BinhDT\n')
            f_predict.write(
    ...
    def train(self,sess, data):
        ...
        for it in range(self.TRAINING_ITERATIONS):
            ...
            _, correct_prediction_train, cost_train = sess.run([self.optimizer, self.correct_prediction, self.cross_entropy], 
            ...
            print('training_accuracy => %.3f, cost value => %.5f for step %d, learning_rate => %.5f' % \
                (float(correct_prediction_train)/np.sum(x_train_binary_mask_batch), cost_train, it, self.learning_rate.eval(session = sess)))
            
            loss_list.append(cost_train)
            accuracy_list.append(float(correct_prediction_train)/np.sum(x_train_binary_mask_batch))

            if it % 100 == 0:
                self.evaluate(sess, data, it + 100 > self.TRAINING_ITERATIONS, self.flag_train)
...
def main():
    ...
    sess = tf.Session()
    sentences = ["Fuck my life!"]
    if (flag_train):
        model.train(sess, data)
    else:
        # data.parse(sentences)
        model.predict(sess, data, True, model.flag_train)
